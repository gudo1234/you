...
const generateProfilePicture = async (mediaUpload) => {
    ...
    if (lib?.sharp?.default) {
        const image = lib.sharp.default(bufferOrFilePath);
        img = await image
            .resize(720, 720, { 
                fit: 'inside', 
        ...
        throw new Error('Sharp no está disponible en la librería');
    }
    return {
        img: img
    };
...
const getStream = async (item, opts) => {
    ...
    if (item.url.toString().startsWith('http://') || item.url.toString().startsWith('https://')) {
        return { stream: await (0, exports.getHttpStreamed)(item.url), type: 'remote' };
    }
...
const getHttpStreamed = (url) => {
    const streamed = got_1.default.stream(url);
    return streamed;
};
exports.getHttpStreamed = getHttpStreamed;
...
const downloadEncryptedContent = async (downloadUrl, { cipherKey, iv }, { startByte, endByte, options } = {}) => {
    ...
    const fetched = (0, exports.getHttpStreamed)(downloadUrl);
